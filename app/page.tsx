...
export default function Home() {
  ...
  useEffect(() => {
    ...
    instance.interimResults = true; instance.continuous = false; recognition.current = instance;
  ...
  async function askCoach(spoken: string) {
    try {
      const response = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ language, message: spoken })
      });
      const data = await response.json();
      const reply = data.reply || data.error || "I couldn't respond right now.";
      setMessages(items => [...items, { role: "coach", text: reply }]);
      if ("speechSynthesis" in window) {
        window.speechSynthesis.cancel();
        const utterance = new SpeechSynthesisUtterance(reply);
        utterance.lang = language === "Hindi" ? "hi-IN" : language === "Spanish" ? "es-ES" : language === "French" ? "fr-FR" : language === "German" ? "de-DE" : language === "Japanese" ? "ja-JP" : "en-US";
        window.speechSynthesis.speak(utterance);
      }
    } catch {
      setMessages(items => [...items, { role: "coach", text: "Connection error. Please try again." }]);
    }
  }
  ...
  function toggleListening() {
    ...
    const locale = language === "English" ? "en-US" : language === "Hindi" ? "hi-IN" : language === "Spanish" ? "es-ES" : language === "French" ? "fr-FR" : language === "German" ? "de-DE" : "ja-JP";
    recognition.current.lang = locale; setTranscript(""); recognition.current.start(); setListening(true);
  }
  ...
  return (
    <main className="shell">
      ...
      <section className="card">
        ...
        <p className="sub">Speak naturally. LinguaCoach will listen, ask the AI, and reply by voice.</p>
        ...
        <div className="transcript">{transcript || (listening ? "Waiting for your voice…" : "Speak and your AI conversation will appear below.")}</div>
        <div className="conversation">
          {messages.map((message, index) => <div key={index} className={message.role === "user" ? "message user" : "message coach"}><span>{message.role === "user" ? "You" : "Coach"}</span><p>{message.text}</p></div>)}
        </div>
